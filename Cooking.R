...
trainSet_tidy <- trainSet %>%
  unnest(ingredients) %>%
  filter(!ingredients %in% stopwords)
...
# Convert list column → plain text
trainSet <- trainSet %>%
  mutate(ingredients_txt = map_chr(ingredients, ~ paste(.x, collapse = " ")))

testSet <- testSet %>%
  mutate(ingredients_txt = map_chr(ingredients, ~ paste(.x, collapse = " ")))

rec <- recipe(cuisine ~ ingredients_txt, data = trainSet) %>%
  step_tokenize(ingredients_txt) %>%
  step_stopwords(ingredients_txt, custom_stopword_source = c("salt", "the")) %>%
  step_tokenfilter(ingredients_txt, max_tokens = 2500) %>%
  step_tfidf(ingredients_txt) %>%
  step_normalize(all_predictors())
...
test_pred <- predict(fit_lgbm, testSet) %>%
...
vroom::vroom_write(
  submission,
  "~/GitHub/Stat 348/WhatsCooking/submission_lgbm_tfidf.csv",
  delim = ","
)
#---------------------------------------
# Overview of Data 
...
library(rjson)
library(jsonlite)
train_raw  <- fromJSON("~/GitHub/Stat 348/WhatsCooking/data/train.json", flatten = TRUE)
test_raw <- fromJSON("~/GitHub/Stat 348/WhatsCooking/data/test.json", flatten = TRUE)
